...
export default function Home() {
    ...
    return (
        ...
        <div>
            ...
            <h3>Encrypted Service Attributes</h3>
            {enSerAtt.length > 0 && enSerAtt.map((attribute) => (
                <h5>
                    {attribute}
                </h5>
            ))}
            <h3>Decrypted Service Attributes</h3>
            {deSerAtt.length > 0 && deSerAtt.map((attribute) => (
                <h5>
                    {attribute}
                </h5>
            ))}
            {identityVerified==true?<h4>Identity token is overall verified by the identity Provider you should verify the admin signature manually to the identity attributes provided to you</h4>:identityVerified==false?
                <h4>Identity token is not yet verified by the identity Provider</h4> :null   
            }
            <h3>Decrypted Identity Attributes</h3>
                ...
                <>
                    {attribute != 'No Value' &&
                        <>
                            ...
                            <p>{verifedArr[index]}</p>
                            <button onClick={()=>verifyHash(attribute,index)}>
                                Verify Hash
                            </button>
                        </>}
                </>
